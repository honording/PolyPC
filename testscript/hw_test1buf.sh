...
do
    ...
    do
        ...
        soft_group=1
        echo "Downloaded $proj_name $buf buf"
        echo "$proj_name $buf buf" >> $output_file
        while [ $soft_group -lt $group ]
        do
            soft_group=$((${soft_group}*2))
        done
        echo "App Group Num: ${soft_group} $buf buf"
        echo "App Group Num: ${soft_group} $buf buf" >> $output_file
        for size in ${data_size[@]}
        do
            cumulative_time=0.0
            for i in `seq 1 ${num_loops}`
            do
                temp_time=$(/bin/${app} ${soft_group} ${size} ${buf} ${2})
