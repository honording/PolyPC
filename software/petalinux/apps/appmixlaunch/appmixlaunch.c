...
int main(int argc, char *argv[])
{
    if (argc != 6) {
        ...
        printf("    fir (fir1buf), vector_mul (vector_mul1buf)\n");
        printf("HW Configurations:\n");
        printf("    1g4s2h, 2g4s4h, 4g4s8h\n");
        return 0;
    }
    int i;
    reg_clr();
    ...
    if (strcmp(argv[4], "vector_add") == 0) {
        ...
        strcpy(hwappname, "vector_mul1buf");
    ...
    strcat(pr_file_path, "/");
    strcat(pr_file_path, hwappname);
    printf("pr_file_path: %s\n", pr_file_path);
    int ret = pr_loader(pr_file_path, &sp.pr_info);
    if (ret < 0) {
        printf("apptes: pr_loader error\n");
        return 0;
    }

    ret = elf_loader(elf_file_path, ELF_START_ADDR, &sp.elf_info);
    if (ret < 0) {
